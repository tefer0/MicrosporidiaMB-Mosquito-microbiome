...
# plot_ordination(ps_object, ordJD, color = "tissue", shape = "mspecies") +
#   geom_point(aes(color=tissue), size = 4) +
...
ovs_vs_mgs.df$sig <- ifelse(ovs_vs_mgs.df$padj<0.2,"yes","no")
ovs_vs_mgs.df$labs <- ifelse(ovs_vs_mgs.df$sig=="yes",rownames(ovs_vs_mgs.df),NA)
ggmaplot(ovs_vs_mgs.df, fdr = 0.05, fc = 2, size = 3,
...
# top20 <- names(sort(taxa_sums(ps_object), decreasing = T))[1:30]
